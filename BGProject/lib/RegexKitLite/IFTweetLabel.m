...
- (void)handleButton:(id)sender
{
	...
	for (NSString *expression in expressions)
	{
		...
		while (match = [enumerator nextObject])
		{
			...
			{
                if ([self.delegate respondsToSelector:@selector(didSelectedIFTweetLabel:)]) {
                    [self.delegate didSelectedIFTweetLabel:match];
                }
			}
		}
	}
...
- (void)createButtonWithText:(NSString *)text withFrame:(CGRect)frame
{
    ...
    [button setBackgroundImage:[UIImage imageNamed:@"userNamebg"] forState:UIControlStateHighlighted];
...
- (void)createButtonsWithText:(NSString *)text atPoint:(CGPoint)point
{
	...
	for (NSString *expression in expressions)
	{
		...
		while (match = [enumerator nextObject])
		{
            ...
            CGSize matchSize = [NSString sizeWithText:match font:font maxSize:CGSizeMake(100, 100)];
...
- (void)createButtons
{
    ...
    CGSize textSize = [NSString sizeWithText:text font:font maxSize:frame.size];
	...
	while (NSMaxRange(scanRange) < textLength)
	{
		...
		else
		{
            ...
            CGSize currentSize =[NSString sizeWithText:scanText font:font maxSize:CGSizeMake(100, 100)];
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame])
	{
		...
		self.normalColor = [UIColor colorWithRed:46/255.0 green:146/255.0 blue:255/255.0 alpha:1.0];
