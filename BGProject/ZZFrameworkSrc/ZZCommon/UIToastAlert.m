...
- (id)initWithMessage:(NSString *)message showDuration:(NSTimeInterval)showDuration fadeDuration:(NSTimeInterval)fadeDuration position:(UIToastAlertPosition)position tintColor:(UIColor *)tintColor lightText:(BOOL)lightText {
  ...
  if (self) {
    ...
    self._messageLabel.backgroundColor = [UIColor grayColor];
    self._messageLabel.textColor = [UIColor whiteColor];
    self._messageLabel.textAlignment = NSTextAlignmentCenter;
    self._messageLabel.shadowOffset = CGSizeMake(0, -1);
    self._messageLabel.shadowColor = [UIColor grayColor];
...
- (void)layoutSubviews {
  ...
  CGSize contraintRect = CGSizeMake(kAlertDefaultWidth - (kAlertDefaultPadding * 2), CGFLOAT_MAX);
    ...
    CGSize sizeOfLabel = [self._messageLabel.text sizeWithFont:[UIFont systemFontOfSize:[UIFont labelFontSize]] constrainedToSize:contraintRect lineBreakMode:NSLineBreakByWordWrapping];
  ...
  switch (self._position) {
    case UIToastAlertPositionTop:
      self.frame = CGRectMake(round(screenRect.size.width/2 - self.frame.size.width/2), screenRect.origin.y + kAlertFadePixelAmount+kMSNaviHight, self.frame.size.width, self.frame.size.height);
      break;
    case UIToastAlertPositionBottom:
      self.frame = CGRectMake(round(screenRect.size.width/2 - self.frame.size.width/2), screenRect.origin.y + screenRect.size.height - self.frame.size.height - kAlertFadePixelAmount-kMSNavigationBarHeight, self.frame.size.width, self.frame.size.height);
      break;
    default:
