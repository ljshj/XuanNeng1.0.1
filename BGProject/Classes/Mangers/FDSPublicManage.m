...
//    UIToastAlert *toast = [UIToastAlert toastForMessage:message];
    ...
    //version for FDS Project
//    UIToastAlert *toast = [UIToastAlert shortToastForMessage:message atPosition:UIToastAlertPositionTop];
//    toast._tintColor = [UIColor grayColor];
//    [toast show:view];
    
    //Version for BG Project
...
{
    // 解析@"/Date(1412845477000)/"
    
    if([dateString hasPrefix:@"/Date("])
    {
        dateString = [dateString substringFromIndex:[@"/Date(" length]];
    }
...
+ (NSString *)fitSmallWithImage:(UIImage *)image
{
    ...
    //FDSPathManager路径管理中心？？只知道它会返回一个图片路径
...
{
    //获取document的路径
    NSString* colorPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    
    //拼接路径
    colorPath =  [colorPath stringByAppendingString:kColorFile];
